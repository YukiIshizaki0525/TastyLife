...
RSpec.describe Comment, type: :system do
  ...
  let(:reply) { build(:comment, :reply, reply_comment: posted_comment.id, user_id: user.id) }
  ...
  describe 'コメント投稿機能' do
    ...
    context 'フォームの入力値が異常' do
      ...
      it 'コメントが100文字を超える場合は投稿不可' do
        visit recipe_path(recipe)
        fill_in "comment_content", with: "a" * 101
        click_button '送信する'
        expect(current_path).to eq recipe_path(recipe)
  ...
  describe 'コメント返信機能' do
    it 'コメント返信可能' do
      posted_comment
      visit recipe_path(recipe)
      fill_in "comment_content", with: reply.content, match: :first
